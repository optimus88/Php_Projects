...
//included for the declared match results display
$flagValue=$_GET["cndt"];
if ($flagValue == 0 )
{
$teamSelectQuery = "SELECT * FROM b17_14312762_tallyworld.tally_match_details WHERE tally_match_flag=0 AND match_details_id=$matchIdValue";
...
{
	$teamSelectQuery = "SELECT * FROM b17_14312762_tallyworld.tally_match_details WHERE tally_match_flag=1 AND match_details_id=$matchIdValue";
	$resultMatchDetails = $ob-> fetch_values($teamSelectQuery);
	while ($row = mysql_fetch_array($resultMatchDetails))
        {
        $match_id = $row['match_details_id'];
        $teamName_1= $row['tally_team1'];
        $teamName_2= $row['tally_team2'];
	...
	include ('match_declared_res.php');
}
?>
